...
class TSP:
    ...
    def find_way(matrix, time_to_run):
        ...
        cost_best = TSP.calculate_route(best, n, matrix)
        cout = 0
        loop_number = 10000
        ...
        while time.time() < deadline:
            ...
            for perm in neighbours:
                if perm not in tabu_list and TSP.calculate_route(perm, n, matrix) < TSP.calculate_route(best_neighbour, n, matrix):
                    best_neighbour = perm
            #update to tabu_list
            ...
            #check len(loop)
            cost_check = TSP.calculate_route(best_neighbour, n, matrix)
        ...
        return np.array(best), TSP.calculate_route(best, n, matrix)
